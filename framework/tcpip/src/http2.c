    ...
    do {
        ...
        case SM_HTTP_PARSE_REQUEST:
            ...
            if (TCPFind(sktHTTP, '\n', 0, false) == 0xffff) { // First line isn't here yet
                ...
                if ((int32_t) (TickGet() - curHTTP.callbackID) > (int32_t) 0) { // A timeout has occurred
        ...
        case SM_HTTP_PARSE_HEADERS:
            ...
            while (1) {
                ...
                if (lenA == 0xffff) { // If not, make sure we can receive more data
                    ...
                    if ((int32_t) (TickGet() - curHTTP.callbackID) > (int32_t) 0) { // A timeout has occured
                        TCPDisconnect(sktHTTP);
                        smHTTP = SM_HTTP_DISCONNECT;
                        isDone = false;
                    }
        ...
        case SM_HTTP_PROCESS_POST:
            ...
            if (TCPIsGetReady(sktHTTP) == curHTTP.callbackPos) {
                if ((int32_t) (TickGet() - curHTTP.callbackID) > (int32_t) 0) { // If a timeout has occured, disconnect
                    TCPDisconnect(sktHTTP);
                    smHTTP = SM_HTTP_DISCONNECT;
                    isDone = false;
                    break;
            ...
            if (curHTTP.httpStatus == HTTP_POST
                ...
                if (c == (uint8_t) HTTP_IO_WAITING) { // return to main app and make sure we don't get stuck by the watchdog
                    curHTTP.callbackPos = TCPIsGetReady(sktHTTP) - 1;
                    break;
                } else if (c == (uint8_t) HTTP_IO_NEED_DATA) { // If waiting for more data
...
static void HTTPHeaderParseCookie(void)
{
    ...
    if (lenB >= (uint16_t) (curHTTP.data + HTTP_MAX_DATA_LEN - curHTTP.ptrData - 2)) { // If not, overflow
        curHTTP.httpStatus = HTTP_OVERFLOW;
        smHTTP = SM_HTTP_SERVE_HEADERS;
        return;
    }
...
uint8_t *HTTPGetArg(uint8_t *cData, uint8_t *cArg)
{
    ...
    while (*cData != '\0') {
        ...
        if (!strcmp((char *) cArg, (char *) cData)) { // Found it, so return parameter
...
uint8_t *HTTPGetROMArg(uint8_t *cData, ROM uint8_t *cArg)
{
    ...
    while (*cData != '\0') {
        // Look for arg at current position
        if (!memcmppgm2ram(cData, (ROM void *) cArg, strlenpgm((ROM char *) cArg) + 1)) { // Found it, so skip to next string
...
static HTTP_READ_STATUS HTTPReadTo(uint8_t cDelim, uint8_t *cData, uint16_t wLen)
{
    ...
    // Read the value
    if (wLen < 2u && cData != NULL) { // Buffer is too small, so read to NULL instead
        curHTTP.byteCount -= TCPGetArray(sktHTTP, NULL, wPos);
        status = HTTP_READ_TRUNCATED;
    } else if (cData == NULL) { // Just remove the data
        curHTTP.byteCount -= TCPGetArray(sktHTTP, NULL, wPos);
        status = HTTP_READ_OK;
    } else if (wPos > wLen - 2) { // Read data, but truncate at max length
...
static HTTP_IO_RESULT HTTPMPFSUpload(void)
{
    ...
    case HTTP_MPFS_UP:
        ...
        if (lenA != 0xffff) { // Found it, so remove all data up to and including
            ...
            curHTTP.byteCount -= lenA;
            if (memcmppgm2ram(c, (ROM void *) "\r\n\r\nMPFS\x02\x01", 10) == 0) { // Read as Ver 2.1
    ...
    case HTTP_MPFS_ERROR:
        ...
        TCPDiscard(sktHTTP);
        if (curHTTP.byteCount < 100u || curHTTP.byteCount > 0x80000000u) { // If almost all data was read, or if we overflowed, then return
            smHTTP = SM_HTTP_SERVE_HEADERS;
            return HTTP_IO_DONE;
        }
        break;
...
void HTTPIncFile(ROM uint8_t *cFile)
{
    ...
    if (curHTTP.callbackPos == 0x00u) { // On initial call, open the file and save its ID
        fp = MPFSOpenROM(cFile);
        if (fp == MPFS_INVALID_HANDLE) { // File not found, so abort
            return;
        }
        ((TCPIP_UINT32_VAL*) & curHTTP.callbackPos)->w[0] = MPFSGetID(fp);
    } else { // The file was already opened, so load up its ID and seek
        fp = MPFSOpenID(((TCPIP_UINT32_VAL *) & curHTTP.callbackPos)->w[0]);
        if (fp == MPFS_INVALID_HANDLE) { // No file handles available, so wait for now
            return;
        }
    ...
    while (wCount > 0u) {
        ...
        if (wLen == 0u) { // If no bytes were read, an EOF was reached
            MPFSClose(fp);
            curHTTP.callbackPos = 0x00;
            return;
        } else { // Write the bytes to the socket
