...
#define CLOSEST_SPBRG_VALUE ((SYS_CLK_FrequencyPeripheralGet()+2ul*BAUD_RATE)/BAUD_RATE/4-1)
#define BAUD_ACTUAL (SYS_CLK_FrequencyPeripheralGet()/(CLOSEST_SPBRG_VALUE+1))
...
#define BAUD_ERROR_PRECENT  ((BAUD_ERROR*100+BAUD_RATE/2)/BAUD_RATE)
    ...
    BAUDCONbits.BRG16 = 1;
    TXSTAbits.BRGH = 1;
    SPBRGH = ((SYS_CLK_FrequencyPeripheralGet() + BAUD_RATE / 2) / BAUD_RATE - 1) >> 8 & 0xFF;
    SPBRG = ((SYS_CLK_FrequencyPeripheralGet() + BAUD_RATE / 2) / BAUD_RATE - 1) & 0xFF;
#else
