...
void MD5Calculate(HASH_SUM * theSum, uint8_t * result)
{
    ...
    if (i > 56u) { // If there's not enough space, then zero fill this and add a new block
        ...
        uint32_t * h3, uint32_t * h4)
{
    ...
    // Set up the w[] vector
    if (lastBlock == data) { // If they're the same, just swap endian-ness
        for (i = 0; i < 16u; i++) {
            back3 = data[3];
            data[3] = data[0];
            data[0] = back3;
    ...
    } else { // Otherwise, copy values in swaping endian-ness as we go
        for (i = 0; i < 16u; i++) {
            w[i].v[3] = *data++;
            w[i].v[2] = *data++;
            w[i].v[1] = *data++;
