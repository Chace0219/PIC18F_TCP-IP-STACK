...
void SSLPeriodic(TCP_SOCKET hTCP, uint8_t id)
{
    ...
    if (sslStub.Flags.bRSAInProgress) {
        if (SSLRSAOperation() == RSA_DONE) { // Move on with the connection
...
uint16_t SSLRxRecord(TCP_SOCKET hTCP, uint8_t id)
{
    ...
    if (sslStub.wRxBytesRem == 0u) {
        ...
        if (sslStub.Flags.bExpectingMAC) { // Receive and verify the MAC
            ...
            // Verify the MAC
            if (memcmp((void *) temp, (void *) &temp[16], 16) != 0) { // MAC fails
    ...
    if (sslStub.Flags.bRemoteChangeCipherSpec && wLen) { // Need to decrypt the data
...
void SSLTxRecord(TCP_SOCKET hTCP, uint8_t id, uint8_t txProtocol)
{
    ...
    if (sslStub.Flags.bLocalChangeCipherSpec) { // Perform the encryption and MAC
        // Sync needed data
        SSLKeysSync(sslStubID);
...
static void SSLTxClientHello(TCP_SOCKET hTCP)
{
    ...
    if (sslStub.Flags.bNewSession) {
        sslStub.idSession = SSLSessionNew();
        if (sslStub.idSession == SSL_INVALID_ID) { // No free sessions, so abort
            return;
        }
    ...
    // Put Session ID
    if (sslStub.Flags.bNewSession) { // Send no session ID
        HSPut(hTCP, 0x00);
    } else { // Send the requested Session ID
        SSLSessionSync(sslStub.idSession);
        HSPut(hTCP, 0x20);
...
static void SSLRxAntiqueClientHello(TCP_SOCKET hTCP)
{
    ...
    HSGet(hTCP, &c);
    if (c != 0x01u) { // This message is not supported, so handshake fails
        TCPRequestSSLMessage(hTCP, SSL_ALERT_HANDSHAKE_FAILURE);
        return;
    }
    ...
    HSGet(hTCP, &c);
    if (c != SSL_VERSION_HI) { // Version is too low, so handshake fails
        TCPRequestSSLMessage(hTCP, SSL_ALERT_HANDSHAKE_FAILURE);
        return;
    }
...
static void SSLRxServerHello(TCP_SOCKET hTCP)
{
    ...
    if (b != 0u) {
        ...
        if (!sslStub.Flags.bNewSession &&
                memcmp((void *) sslSession.sessionID, (void *) sessionID, 32) == 0) { // Session restart was accepted
...
static void SSLRxServerCertificate(TCP_SOCKET hTCP)
{
    ...
    case RX_SERVER_CERT_START:
        ...
        if (len == 0xFFFFu) { // If not found, clear most of buffer and return to wait
    ...
    case RX_SERVER_CERT_FIND_KEY:
        ...
        if (len == 0xFFFFu) { // Not found, so clear and return
...
static void SSLRxFinished(TCP_SOCKET hTCP)
{
    ...
    if (memcmp((void *) rxHash, (void *) expectedHash, 16) != 0) { // Handshake hash fails
        TCPRequestSSLMessage(hTCP, SSL_ALERT_HANDSHAKE_FAILURE);
    }
    ...
    if (memcmp((void *) rxHash, (void *) expectedHash, 20) != 0) { // Handshake hash fails
        TCPRequestSSLMessage(hTCP, SSL_ALERT_HANDSHAKE_FAILURE);
    }
...
static bool SSLStubAlloc(void)
{
    ...
    for (i = 0; i != MAX_SSL_CONNECTIONS; i++) {
        if (!(isStubUsed & masks[i])) { // Stub is free, so claim it
            isStubUsed |= masks[i];
...
static void SSLHashAlloc(uint8_t *id)
{
    ...
    for (i = 0; i != MAX_SSL_HASHES; i++) {
        if (!(isHashUsed & masks[i])) { // Hash is free, so claim it
            isHashUsed |= masks[i];
...
static void SSLBufferAlloc(uint8_t *id)
{
    ...
    for (i = 0; i != MAX_SSL_BUFFERS; i++) {
        if (!(isBufferUsed & masks[i])) { // Buffer is free, so claim it
...
static uint8_t SSLSessionNew(void)
{
    ...
    for (id = 0; id != MAX_SSL_SESSIONS; id++) {
        if (sslSessionStubs[id].tag.Val == 0u) { // Unused session, so claim immediately
            break;
        }

        // Check how old this session is
        age = now - sslSessionStubs[id].lastUsed;
        if (age > oldest) { // This is now the oldest one
            oldest = age;
            oldestID = id;
        }
    }
